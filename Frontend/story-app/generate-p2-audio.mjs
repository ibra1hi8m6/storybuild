// Generates p2-q1..5.wav — Arabic letter name audio for Part 2 of placement test.
// Run from story-app folder: node generate-p2-audio.mjs
...
const clips = [
  { file: 'p2-q1', text: 'الألف' },
  { file: 'p2-q2', text: 'الباء' },
  { file: 'p2-q3', text: 'التاء' },
  { file: 'p2-q4', text: 'الثاء' },
  { file: 'p2-q5', text: 'الجيم' },
];
...
async function generateClip(file, text, retries = 5) {
  ...
  for (let attempt = 0; attempt < retries; attempt++) {
    ...
    if (!pcm) {
      // finishReason OTHER usually means transient model issue — wait and retry
  ...
  throw new Error(`Failed to generate audio for "${text}" after ${retries} attempts`);
}
